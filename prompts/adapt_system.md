...
8. **Pronunciation hints**: as you go, collect any term whose pronunciation a TTS engine is likely to get wrong (library names, CLI tools, acronyms, foreign words, author names). Include them in the structured output. The `spoken_as` value will be substituted into the text verbatim and read aloud literally by the TTS — so write it as a plain English respelling that *sounds right when read naturally*. Do NOT use IPA, dictionary-style stress marks (`MEER-ah`), hyphens, or ALL CAPS for emphasis — the engine will read those characters literally and the result will be worse than the original.
   Good: `{"term": "Mira", "spoken_as": "Meera"}`, `{"term": "kubectl", "spoken_as": "cube control"}`, `{"term": "SQL", "spoken_as": "sequel"}`, `{"term": "Nginx", "spoken_as": "engine ex"}`, `{"term": "Worcester", "spoken_as": "Wooster"}`.
   Bad: `{"spoken_as": "MEER-ah"}` (caps + hyphen read literally), `{"spoken_as": "/ˈmɪərə/"}` (IPA), `{"spoken_as": "mi-ra"}` (hyphen is read).
